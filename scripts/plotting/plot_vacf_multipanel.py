...
for j in range(len(lambdas)):
    ...
    for i in range(len(taus)):
        ...
        for k in range(len(vas)):
            ...
            data = np.load(thedir + '/velocity_time_corr_avg.npz')
            ...
            axs[j][i].set_ylim([-0.2,1.0])
            #axs[j][i].set_xlim([-0.3, 0.3])
            

        if i==0:
            axs[j][i].set_ylabel(r'$C_{vv}(t)$')
        if j==len(lambdas)-1:
            axs[j][i].set_xlabel(r'$t$')
    
    #axs[j].set_yscale('log')
axs[0][0].legend(fontsize=8, loc='upper right')
plt.savefig('plots/2d/vacf_network_multipanel_Nx=%d_Ny=%d.png' % (Nx, Ny), dpi=600, bbox_inches='tight')
plt.close()
