...
for Lambda in lambdas:
    ...
    for i in range(len(taus)):
        ...
        sq_data = np.load(thedir + '/sq_var_avg.npz')
        sqavg = sq_data['sq_var_vals_1d_avg']
        sqerr = sq_data['sq_var_vals_1d_stderr']
    ...
    plt.ylabel(r'$\langle (\delta S(q))^2 \rangle$')
    plt.legend(fontsize=10)
    plt.xlim([0.0,1.0])
    plt.savefig('plots/2d/phi=%f/sq_var_1d_vary_tau_va=%f_Lambda=%f_Lx=%.01f_Ly=%.01f.png' % (phi, va, Lambda, Lx, Ly), dpi=300, bbox_inches='tight')
...
#plt.yscale('log')
#plt.tight_layout()
plt.savefig('plots/2d/sq_var_quenched_vary_lambda_phi=%.01f_va=%.01f_Lx=%.01f_Ly=%.01f.png' % (phi, va, Lx, Ly), dpi=300, bbox_inches='tight')
