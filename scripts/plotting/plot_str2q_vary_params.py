...
for j in range(len(lambdas)):
    ...
    for i in range(len(taus)):
        ...
        str2q_data = np.load(thedir + '/strain_corr_q_avg.npz')
        str2qavg = str2q_data['str2q_vals_1d_nlast_avg']
        str2qerr = str2q_data['str2q_vals_1d_nlast_stderr']
        q1d = str2q_data['qvals_1d_avg']
        themax = np.max(str2qavg)
        theargmax = np.argmax(str2qavg)
        taulabel = r'$\tau_a=%.01f$' % taus[i]
        if tau==float('inf'):
            taulabel = r'$\tau_a=\infty$'
        axs[j].plot(q1d,str2qavg,color=colors_tau[i], label=taulabel)
        #axs[j].fill_between(q1d, str2qavg-2*str2qerr, str2qavg+2*str2qerr, color=colors_tau[i], alpha=0.4)
    ...
    axs[j].set_ylabel(r'$\langle | s(q) |^2 \rangle$')
...
plt.savefig('plots/2d/str2q_1d_multipanel_vary_tau_va=%f_Nx=%d_Ny=%d.png' % (va, Nx, Ny), dpi=300, bbox_inches='tight')
plt.close()
...
for i in range(len(lambdas)):
    Lambda = lambdas[i]
    thedir = basedir + '/quenched/lambda=%f/Nx=%d_Ny=%d/nx=%d_ny=%d/interpolation=%s/%s/%s/' % (Lambda, Nx, Ny, nx, ny, interpolation, compressibility, cov_type)
    #data = str2q.rebin_str2q(thedir, nbins=num_bins)
    #str2q_data = stats.get_postprocessed_stats(data)
    str2q_data = np.load(thedir + '/strain_corr_q_avg.npz')
    str2qavg = str2q_data['str2q_vals_1d_nlast_avg']
    str2qerr = str2q_data['str2q_vals_1d_nlast_stderr']
    q1d = str2q_data['qvals_1d_avg']
    themax = np.max(str2qavg)
    theargmax = np.argmax(str2qavg)
    axs.plot(q1d,str2qavg/str2qavg[0], label=r'$\lambda_a=%.01f$' % Lambda, color=colors[i])
    #axs.fill_between(q1d, str2qavg+2*str2qerr, str2qavg-2*str2qerr, alpha=0.4, color=colors[i])
    ...
    #axs.scatter(q1d[1:-1],str2qavg[1:], label=r'$\lambda_a=%.01f$' % Lambda)
    #axs.plot(q1d[1:-1],str2qavg[1:], label=r'$\lambda_a=%.01f$' % Lambda)

    axs.set_xlabel(r'$q\sigma$')
    axs.set_ylabel(r'$\langle | s(q) |^2 \rangle$')
...
#plt.yscale('log')
#plt.tight_layout()
plt.savefig('plots/2d/str2q_1d_quenched_vary_lambda_va=%.02f_Nx=%d_Ny=%d.png' % (va, Nx, Ny), dpi=300, bbox_inches='tight')
plt.close()
