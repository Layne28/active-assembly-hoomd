...
for i in range(len(lambdas)):
    for j in range(len(taus)):
        ...
        corr_arr = data['spatial_corr_norm_avg'][:,1]
        corr_err = data['spatial_corr_norm_stderr'][:,1]
...
plt.savefig('plots/2d/pressure_spatial_cross_corr_%s.png' % potential)
...
for i in range(len(lambdas)):
    for j in range(len(taus)):
        ...
        if 'spatial_corr_norm_avg' in data.keys():
            dist_arr = data['spatial_corr_avg'][:,0] #data output got messed up so use these (accidentally "normalized" the x values in norm_avg)
            corr_arr = data['spatial_corr_abs_norm_avg'][:,1]
            corr_err = data['spatial_corr_abs_norm_stderr'][:,1]

            print(corr_arr[0], data['one_point_corr_abs_norm_avg'])
...
plt.savefig('plots/2d/pressure_abs_spatial_cross_corr_%s.png' % potential)