...
for j in range(len(lambdas)):
    ...
    for i in range(len(taus)):
        ...
        axs[j].plot(q1d,p2qavg/p2qavg[0],color=colors_tau[i], label=taulabel)
        #axs[j].fill_between(q1d, p2qavg-2*p2qerr, p2qavg+2*p2qerr, color=colors_tau[i], alpha=0.4)
    ...
    axs[j].set_ylabel(r'$\langle | p(q) |^2 \rangle$')
    axs[j].set_xlim([0,0.5])
    axs[j].set_title(r'$\lambda_{\text{a}}=%.01f$' % Lambda)
    #axs[j].set_yscale('log')
axs[-1].legend(fontsize=8, loc='upper right')
axs[-1].set_xlabel(r'$q$')
plt.savefig('plots/2d/p2q_1d_multipanel_vary_tau_va=%f_Nx=%d_Ny=%d.png' % (va, Nx, Ny), dpi=300, bbox_inches='tight')
plt.close()
...
for i in range(len(lambdas)):
    ...
    themax = np.max(p2qavg)
    theargmax = np.argmax(p2qavg)
    axs.plot(q1d,p2qavg/p2qavg[0], label=r'$\lambda_a=%.01f$' % Lambda, color=colors[i])
    #axs.fill_between(q1d, p2qavg+2*p2qerr, p2qavg-2*p2qerr, alpha=0.4, color=colors[i])
    ...
    axs.set_ylabel(r'$\langle | p(q) |^2 \rangle$')
...
#plt.yscale('log')
#plt.tight_layout()
plt.savefig('plots/2d/p2q_1d_quenched_vary_lambda_va=%.01f_Nx=%d_Ny=%d.png' % (va, Nx, Ny), dpi=300, bbox_inches='tight')
plt.close()
