...
for va in "${vas[@]}"; do
    ...
    srun parallel -k --lb --jobs $njob "python $run_dir/trajectory_stats.py $SCRATCH/active-assembly-hoomd/manyseed/${potential}/${d}d/kT={1}/phi={2}/va=${va}/{3}/lambda={4}/Lx=${Lx}_Ly=${Lx}/nx=${nx}_ny=${nx}/interpolation=${interp}/${compressibility}/${cov_type}/ csd csd postprocessed > $SCRATCH/active-assembly-hoomd/log/csd_kT={1}_phi={2}_va=${va}_{3}_lambda={4}_avg.out" \
                            ::: ${kTs[@]} \
                            ::: ${phis[@]} \
                            ::: ${taus[@]} \
                            ::: ${lambdas[@]}  | tr -d \''"\' &
